...
extension ViewController {
    ...
    private func renamePerson(at indexPath: IndexPath) {
        ...
        let saveAction = UIAlertAction(title: "Save", style: .default) { [weak self, weak ac] _ in
            ...
            self?.people[indexPath.item].rename(to: newName)
    ...
    private func deletePerson(at indexPath: IndexPath) {
        ...
        let deleteAction = UIAlertAction(title: "Delete", style: .destructive) { [weak self] _ in
            ...
            guard self.deleteImage(self.people[indexPath.item].image) else { return }
    ...
    private func deleteImage(_ imageName: String) -> Bool {
        let fm = FileManager()
        let dir = getDocumentsDirectory().appendingPathComponent(imageName)
